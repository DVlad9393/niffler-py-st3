...
class NewSpendingPage(BasePage):
    def __init__(self, page: Page) -> None:
        ...
        self.title = Title(
            page,
            locator='//h2[contains(text(), "Add new spending")]',
            name="Add new spending title",
        )

        self.amount_input = Input(page, locator="#amount", name="Amount input")
        ...
        self.currency_option_rub = ListItem(
            page,
            locator='ul[role="listbox"] li:has-text("RUB")',
            name="Currency RUB option",
        )

        self.category_input = Input(page, locator="#category", name="Category input")

        self.date_input = Input(page, locator='input[name="date"]', name="Date input")
        ...
        self.cancel_button = Button(page, locator="#cancel", name="Cancel button")

        self.add_button = Button(page, locator="#save", name="Add button")

        self.error_amount_message = Text(
            page,
            locator='//span[contains(text(), "Amount has to be")]',
            name="Error message",
        )

        self.error_no_category_message = Text(
            page,
            locator='//span[contains(text(), "Please choose category")]',
            name="Error message",
        )
