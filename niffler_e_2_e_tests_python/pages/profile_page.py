...
class ProfilePage(BasePage):
    def __init__(self, page: Page) -> None:
        ...
        self.name_input = Input(page, locator='input[name="name"]', name="Name")

        self.save_button = Button(page, locator='button[type="submit"]', name="Save")
        ...
        self._categories_chips = self.page.locator(
            'div[class*="MuiBox-root css-1lekzkb"]'
        )

        self.category_placeholder = Input(
            self.page, 'input[placeholder="Edit category"]', name="Category placeholder"
        )
        ...
        self.unarchive_category_submit_button = Button(
            page,
            locator='xpath=//button[text() = "Unarchive"]',
    ...
    def add_category(self, category: str) -> None:
        with allure.step(f'Adding category "{category}"'):
            ...
            food_button = Button(
                self.page, f'//div/span[text() = "{category}"]', "Категория"
            )
            food_button.should_be_enabled()
            food_button.should_be_visible()

    def edit_category_name(
        self, old_category_name: str, new_category_name: str
    ) -> None:
        with allure.step(
            f'Editing category "{old_category_name}" -> "{new_category_name}"'
        ):
            chips_button_old_category = Button(
                self.page,
                f'//div/span[text() = "{old_category_name}"]',
                "Старая категория",
            )
            ...
            self.page.keyboard.press("Enter")
            chips_button_new_category = Button(
                self.page,
                f'//div/span[text() = "{new_category_name}"]',
                "Новая категория",
            )
    ...
    def archive_category(self, category: str) -> None:
        with allure.step(f'Archiving category "{category}"'):
            ...
            chips_button_category = Button(
                self.page, f'xpath=//div/span[text() = "{category}"]', "Категория"
            )
