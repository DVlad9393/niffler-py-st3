...
class CategoriesApiClient(BaseApiClient):
    def get_all_categories(self, exclude_archived: bool = False) -> list[CategoryDTO]:
        ...
        :param exclude_archived: Исключать ли архивированные категории (по умолчанию False).
...
class SpendApiClient(BaseApiClient):
    def get_all_spends(
        ...
        with allure.step("Get all spends"):
            ...
            resp = self.session.get(
                "/api/spends/all", params=params, headers=self.headers
            )
            self.raise_for_status(resp)
    ...
    def add_spending(
        ...
        with allure.step("Add spending"):
            ...
            resp = self.session.post(
                "/api/spends/add", json=payload, headers=self.headers
            )
            self.raise_for_status(resp)
